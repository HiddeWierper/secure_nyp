...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$password || !$password_confirm) {
        ...
        // Check token geldig en niet verlopen
        $stmt = $db->prepare("SELECT id, invite_expiry FROM users WHERE invite_token = ?");
        $stmt->execute([$token]);
        $user = $stmt->fetch(PDO::FETCH_ASSOC);

        if (!$user) {
            $error = 'Ongeldige of verlopen link.';
        } elseif (time() > $user['invite_expiry']) {
            $error = 'De link is verlopen.';
        } else {
            // Update wachtwoord en verwijder token
            $password_hash = password_hash($password, PASSWORD_DEFAULT);
            $stmt = $db->prepare("UPDATE users SET password_hash = ?, invite_token = NULL, invite_expiry = NULL WHERE id = ?");
...
<head>
  <meta charset="UTF-8" />
  <title>Wachtwoord instellen - New York Pizza</title>
    ...
    <div class="glass-card rounded-2xl p-6 sm:p-8">
      ...
      <div class="text-center mb-8">
        ...
        <p class="text-gray-600 text-sm sm:text-base">
          Stel je wachtwoord in om aan de slag te gaan
        </p>
      </div>
      ...
      <?php if ($success): ?>
        ...
        <div class="alert-success px-4 py-3 rounded-xl mb-6" role="alert">
          <div class="flex items-center">
            <i class="fas fa-check-circle mr-3 text-blue-600 success-checkmark"></i>
            <span class="font-medium">Je wachtwoord is succesvol ingesteld!</span>
          </div>
        </div>
        ...
        <div class="form-group text-center">
          <div class="mb-6">
            <div class="w-20 h-20 mx-auto bg-blue-100 rounded-full flex items-center justify-center mb-4">
              <i class="fas fa-check text-blue-600 text-3xl success-checkmark"></i>
            </div>
      ...
      <?php else: ?>
        <?php if ($token): ?>
          <!-- Form -->
          <form method="post" action="" class="space-y-6" id="setPasswordForm">
            ...
            <div class="form-group">
              ...
              <div class="relative">
                ...
                <i class="fas fa-key absolute left-4 top-1/2 transform -translate-y-1/2 text-blue-600 opacity-70 input-icon"></i>
                <button type="button" 
                        id="togglePassword1"
            ...
            <div class="form-group">
              ...
              <div class="relative">
                <input type="password" 
                       ...
                       class="input-field w-full px-4 py-3 rounded-xl focus:outline-none text-sm sm:text-base pl-12 pr-12" />
                <i class="fas fa-check-double absolute left-4 top-1/2 transform -translate-y-1/2 text-blue-600 opacity-70 input-icon"></i>
                <button type="button" 
                        id="togglePassword2"
                        class="absolute right-4 top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-blue-600 transition-colors">
                  <i class="fas fa-eye"></i>
        ...
        <?php else: ?>
          ...
          <div class="alert-error px-4 py-3 rounded-xl mb-6" role="alert">
            <div class="flex items-center">
              <i class="fas fa-exclamation-circle mr-3 text-red-600"></i>
              <span class="font-medium">Ongeldige of ontbrekende link.</span>
            </div>
          </div>
    ...
    if (form) {
      form.addEventListener('submit', function(e) {
        ...
        if (submitBtn) {
          ...
          // Show loading state
          submitBtn.innerHTML = '<i class="fas fa-spinner fa-spin mr-2"></i>Instellen...';
